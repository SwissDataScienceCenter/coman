...
pub struct GetHeadFilesystemSystemNameOpsHeadGetRequest {
    ...
    /// the respective argument through `bytes` or `lines`. ## Schema
    /// - Location: Query
...
pub struct ServiceAccount {
    ...
    ///Service account secret token. You can give directly the content or the file path using
    /// `'secret_file:/path/to/file'`.
    #[validate(length(min = 1))]
    pub secret: String,
}
